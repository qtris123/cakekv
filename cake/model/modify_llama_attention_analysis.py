...
def store_attention_data(layer_idx, step, attention_weights, entropy_score, variance_score, pref_score, evict_score, budget):
    """Store attention data for analysis"""
    global attention_data
    
    if layer_idx not in attention_data['layer_attention_scores']:
        attention_data['layer_attention_scores'][layer_idx] = []
        attention_data['layer_entropy_scores'][layer_idx] = []
        attention_data['layer_variance_scores'][layer_idx] = []
        attention_data['preference_scores'][layer_idx] = []
        attention_data['eviction_scores'][layer_idx] = []
        attention_data['layer_budgets'][layer_idx] = []
    
    # Store attention weights (detach and move to CPU to save memory)
    if attention_weights is not None:
        attention_data['layer_attention_scores'][layer_idx].append(attention_weights.detach().cpu())
    
    # Store scores
    if entropy_score is not None:
        attention_data['layer_entropy_scores'][layer_idx].append(entropy_score)
    if variance_score is not None:
        attention_data['layer_variance_scores'][layer_idx].append(variance_score)
    if pref_score is not None:
        attention_data['preference_scores'][layer_idx].append(pref_score)
    if evict_score is not None:
        attention_data['eviction_scores'][layer_idx].append(evict_score.detach().cpu())
    if budget is not None:
        attention_data['layer_budgets'][layer_idx].append(budget)
    
    # Store step info
    if step not in attention_data['inference_steps']:
        attention_data['inference_steps'].append(step)


def llama_attn_forward_cake_with_analysis(
    self,
    ...
    # Initialize variables for storing attention data
    attention_weights_to_store = None
    entropy_score = None
    variance_score = None
    pref_score = None
    evict_score = None
    ...
    if self.config.prefill[self.layer_idx]:
        ...
        attention_weights_to_store = tmp_attn_weights.clone()
        ...
        entropy_score = disp.cpu().numpy()
        variance_score = var.cpu().numpy()
    ...
    step = getattr(self, '_inference_step', 0)
    store_attention_data(self.layer_idx, step, attention_weights_to_store, entropy_score, variance_score, pref_score, evict_score, budget)
...
def llama_model_forward_cake_with_analysis(
    self,
